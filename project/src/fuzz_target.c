...
void process_input(char *buf, int len) {
    ...
    /*
     * Keep the generic stack bug away from the FUZZ/CRASH prefix paths.
     * Otherwise AFL quickly finds many long-input stack crashes and those
     * shallow crashes mask the deeper heap/null bugs during a short demo.
     */
    if (len > 2 && buf[0] != 'F' && buf[0] != 'C') {
