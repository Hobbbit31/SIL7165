...
class Instrument():
    ...
    def get_mem_instrumentation(self, acsz, instruction, midx, free, is_leaf):
        ...
        # XXX: Bug in capstone?
        # For some instructions, capstone reports the wrong operand index for the
        # memory access. We override midx=1 so the ASan check looks at the correct operand.
        # We added "rep movs" here (Extension 1) because it has the same issue.
        if any([
            instruction.mnemonic.startswith(x) for x in
            ["sar", "shl", "shl", "stos", "shr", "rep stos", "rep movs"]
        ]):
            midx = 1
        ...
        # --- EXTENSION 1: rep movs (memcpy) ASan fix ---
        # The original RetroWrite skipped rep movs instructions entirely (there was a TODO).
        # rep movsb copies %rcx bytes from (%rsi) -> (%rdi).
        # We set the destination operand to (%rdi) so the main code path checks dst start.
        # The remaining 3 boundary checks (dst end, src start, src end) are added below.
        # Set DISABLE_REP_FIX=1 to turn this off and show the original (broken) behavior.
        if instruction.mnemonic.startswith("rep movs") and os.environ.get("DISABLE_REP_FIX") != "1":
            is_rep_movs = True
            lexp = "(%rdi)"  # destination operand — rep movs always writes to (%rdi)
        elif instruction.mnemonic.startswith("rep stos"):
            is_rep_stos = True
        ...
        # --- EXTENSION 1: Add the 3 extra boundary checks for rep movs ---
        # rep movsb copies %rcx bytes from (%rsi) to (%rdi). To cover the copy,
        # we need to check all 4 boundaries:
        #   1. Destination start (%rdi)        — already checked above by the main code path
        #   2. Destination end   (%rdi, %rcx)  — does the copy overflow the dest buffer?
        #   3. Source start      (%rsi)         — is the source buffer valid?
        #   4. Source end        (%rsi, %rcx)   — does the copy read past the source?
        #
        # Important: we copy from base_codecache (the original template) each time,
        # NOT from the accumulated codecache. If we don't do this, the format() calls
        # produce duplicate labels and the assembler throws errors.
        # (We found this bug during testing — took a while to figure out!)
        if is_rep_movs:
            base_codecache = copy.copy(codecache)
            original_exit = copy.copy(
                sp.MEM_EXIT_LABEL)[0].format(addr=instruction.address)

            # Check 2: destination end — (%rdi + %rcx) = last byte written
            dst_end_args = copy.copy(args)
            dst_end_args["lexp"] = "(%rdi, %rcx)"
            dst_end_args["addr"] = "%d_2" % (instruction.address)
            check2 = copy.copy(base_codecache).format(**dst_end_args)
            new_exit2 = copy.copy(sp.MEM_EXIT_LABEL)[0].format(
                addr="%d_2" % (instruction.address))
            check2 = check2.replace(original_exit, new_exit2)
            codecache = codecache + "\n" + check2

            # Check 3: source start — (%rsi) = first byte read
            src_start_args = copy.copy(args)
            src_start_args["lexp"] = "(%rsi)"
            src_start_args["addr"] = "%d_3" % (instruction.address)
            check3 = copy.copy(base_codecache).format(**src_start_args)
            new_exit3 = copy.copy(sp.MEM_EXIT_LABEL)[0].format(
                addr="%d_3" % (instruction.address))
            check3 = check3.replace(original_exit, new_exit3)
            codecache = codecache + "\n" + check3

            # Check 4: source end — (%rsi + %rcx) = last byte read
            src_end_args = copy.copy(args)
            src_end_args["lexp"] = "(%rsi, %rcx)"
            src_end_args["addr"] = "%d_4" % (instruction.address)
            check4 = copy.copy(base_codecache).format(**src_end_args)
            new_exit4 = copy.copy(sp.MEM_EXIT_LABEL)[0].format(
                addr="%d_4" % (instruction.address))
            check4 = check4.replace(original_exit, new_exit4)
            codecache = codecache + "\n" + check4
    ...
    def instrument_mem_accesses(self):
        for _, fn in self.rewriter.container.functions.items():
            ...
            for idx, instruction in enumerate(fn.cache):
                ...
                # --- EXTENSION 1: Handle rep movs (memcpy) instrumentation ---
                # The original RetroWrite had a TODO here — it just skipped rep movs.
                # This means memcpy-style overflows were not cleanly reported by ASan.
                #
                # Our fix: we intercept rep movs here and add ASan shadow memory checks
                # for the destination start. The get_mem_instrumentation() method then
                # adds 3 more checks (dst end, src start, src end) via the is_rep_movs flag.
                #
                # Set DISABLE_REP_FIX=1 env var to turn this off for demo comparison.
                if instruction.mnemonic.startswith("rep movs"):
                    if os.environ.get("DISABLE_REP_FIX") == "1":
                        print("[!] DISABLE_REP_FIX=1: Skipping rep movs instrumentation (Extension 1 OFF)")
                        continue
                    acsz = 1  # rep movsb operates on single bytes
                    if idx in fn.analysis['free_registers']:
                        free_registers = fn.analysis['free_registers'][idx]
                    else:
                        free_registers = list()

                    # This call instruments the destination start check (%rdi).
                    # Inside get_mem_instrumentation(), the is_rep_movs flag triggers
                    # 3 additional checks for dst end, src start, and src end.
                    iinstr_dst_start = self.get_mem_instrumentation(
                        acsz, instruction, midx if midx is not None else 0,
                        free_registers, is_leaf)
                    self.memcheck_sites[fn.start].append(idx)
                    instruction.instrument_before(iinstr_dst_start)
                    continue

                # XXX: rep stos handled via is_rep_stos flag + copycache duplication below
