...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\textcolor{gray}{RetroWrite Reproduction}}
\fancyhead[R]{\small\textcolor{gray}{SIL7165 -- Mid-Term Report}}
...
\midrule
\rowcolor{rowgray} QEMU \cite{qemu} & DBT & Yes & No (10--100x) & Yes & Very slow for fuzzing \\
Valgrind \cite{valgrind} & DBT & Yes & No (2--300x) & Yes & Too slow for fuzzing \\
\rowcolor{rowgray} Intel PIN \cite{pin} & DBT & Yes & No (2--5x) & Yes & Runtime overhead \\
DynamoRIO \cite{dynamorio} & DBT & Yes & No (2--5x) & Yes & Runtime overhead \\
\rowcolor{rowgray} Uroboros \cite{uroboros} & Static & No & Yes & No & Wrong heuristics \\
ramblr \cite{ramblr} & Static & No & Yes & Partial & Still uses heuristics \\
\rowcolor{rowgray} DynInst \cite{dyninst} & Static & Partial & Yes & Partial & Unsound instrumentation \\
\rowcolor{accent!10} \textbf{RetroWrite} \cite{retrowrite} & \textbf{Static} & \textbf{Yes} & \textbf{Yes} & \textbf{Yes} & \textbf{Only 64-bit PIC} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tikzpicture}[
    node distance=0.4cm,
    box/.style={draw=accent, fill=accent!8, rounded corners=4pt, minimum height=1cm, minimum width=2.2cm, align=center, font=\small\bfseries},
    io/.style={draw=primary, fill=primary!12, rounded corners=4pt, minimum height=1cm, minimum width=1.8cm, align=center, font=\small\bfseries},
    arr/.style={->, thick, color=accent!70, >=stealth}
]
\node[io] (input) {Binary};
\node[box, right=of input] (pre) {1. Pre-\\processing};
\node[box, right=of pre] (sym) {2. Symbol-\\ization};
\node[box, right=of sym] (inst) {3. Instru-\\mentation};
\node[box, right=of inst] (opt) {4. Optim-\\ization};
\node[box, right=of opt] (asm) {5. Re-\\assembly};
\node[io, right=of asm] (output) {Instrumented\\Binary};
...
\midrule
\rowcolor{rowgray} Heap & Individual object & Individual object \\
Stack & Individual, 32-byte & Stack-frame, 8-byte \\
\rowcolor{rowgray} Global & Individual, padded to 64-byte & No redzone \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
\rowcolor{rowgray} Heap buffer overflow & \textcolor{danger}{Not detected (silent)} & \textcolor{success}{Detected} \\
Use-after-free & \textcolor{danger}{Not detected (silent)} & \textcolor{success}{Detected} \\
\bottomrule
\end{tabular}
\end{table}
...
We used RetroWrite to rewrite the bzip2 binary (105KB). RetroWrite successfully:
\begin{itemize}
    \item Disassembled bzip2 into assembly (807KB assembly file)
    \item Reassembled it into a working binary (101KB)
...
\subsection{Experiment 3: Extended ASan Testing}

We wrote our own test program (\texttt{asan\_test.c}) with four distinct memory bugs to further validate RetroWrite's ASan capabilities beyond the basic demo.

\begin{table}[H]
\centering
\caption{Extended ASan detection results on our custom test program}
\label{tab:asan_extended}
\begin{tabular}{lcc}
\toprule
\textbf{Bug Type} & \textbf{Original Binary} & \textbf{RetroWrite + ASan} \\
\midrule
\rowcolor{rowgray} Heap buffer overflow & \textcolor{danger}{Not detected (silent)} & \textcolor{success}{Detected} \\
Use-after-free & \textcolor{danger}{Not detected (silent)} & \textcolor{success}{Detected} \\
\rowcolor{rowgray} Stack buffer overflow & \textcolor{danger}{Not detected (crash)} & \textcolor{success}{Detected} \\
Double free & \textcolor{danger}{Not detected (crash)} & \textcolor{success}{Detected} \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Experiment 4: Attack Surface Analysis}

We also performed a systematic attack analysis on our fuzz target program (\texttt{fuzz\_target.c}). We tested 10 different attack types and classified each by whether it was possible, whether the original binary crashed, and whether ASan detected it.

\begin{table}[H]
\centering
\caption{Attack surface analysis on fuzz\_target.c}
\label{tab:attack_analysis}
\small
\begin{tabular}{lccc}
\toprule
\textbf{Attack Type} & \textbf{Possible?} & \textbf{Original Crashes?} & \textbf{ASan Detects?} \\
\midrule
\rowcolor{danger!8} Heap buffer overflow & \textcolor{danger}{Yes} & No (silent corruption) & \textcolor{success}{Yes} \\
\rowcolor{danger!8} Null pointer deref & \textcolor{danger}{Yes} & Yes (segfault) & \textcolor{success}{Yes} \\
\rowcolor{danger!8} Stack buffer overflow & \textcolor{danger}{Yes} & Yes (segfault) & \textcolor{success}{Yes} \\
\midrule
\rowcolor{rowgray} Format string & No & --- & --- \\
Use-after-free & No & --- & --- \\
\rowcolor{rowgray} Double free & No & --- & --- \\
Integer overflow & No & --- & --- \\
\rowcolor{rowgray} Command injection & No & --- & --- \\
Information leak & No & --- & --- \\
\rowcolor{rowgray} Race condition & No & --- & --- \\
\bottomrule
\end{tabular}
\end{table}

RetroWrite's ASan instrumentation detected all three possible memory safety attacks, including the heap buffer overflow that silently corrupts memory without crashing -- the most dangerous type of bug since the attacker remains undetected.

\subsection{Experiment 5: AFL Fuzzing Performance}
...
\begin{tabular}{lcc}
\toprule
\textbf{Method} & \textbf{Speed (exec/sec)} & \textbf{Source} \\
\midrule
\rowcolor{rowgray} Source-level AFL (best possible) & 4790 & Our measurement \\
\rowcolor{accent!10} \textbf{RetroWrite AFL (binary-only)} & \textbf{4244 (88.6\%)} & Our measurement \\
\rowcolor{rowgray} QEMU AFL & $\sim$800 & Reported in paper \\
\bottomrule
\end{tabular}
\end{table}

RetroWrite achieved 88.6\% of source-level AFL speed without needing the source code. This is consistent with the paper's claim of near-native performance. Note that the QEMU AFL figure is from the original paper, as we did not set up QEMU-based fuzzing. The QEMU-based approach is roughly 5x slower.

\subsection{Comparison with Existing Solutions}

\begin{table}[H]
\centering
\caption{Quantitative comparison with existing solutions (overhead figures from the RetroWrite paper \cite{retrowrite})}
\label{tab:comparison}
\begin{tabular}{lccc}
\toprule
\textbf{Tool} & \textbf{Needs Source?} & \textbf{Runtime Overhead} & \textbf{Bug Detection} \\
\midrule
\rowcolor{rowgray} Source ASan & Yes & 1.73x & Best \\
\rowcolor{accent!10} \textbf{RetroWrite ASan} & \textbf{No} & \textbf{1.65x} & \textbf{Good} \\
\rowcolor{rowgray} Valgrind memcheck & No & 20--300x & Lower \\
QEMU + AFL & No & 10--100x & Low throughput \\
\rowcolor{accent!10} \textbf{RetroWrite + AFL} & \textbf{No} & $\boldsymbol{\sim}$\textbf{1x} & \textbf{Near source-level} \\
\bottomrule
\end{tabular}
\end{table}
